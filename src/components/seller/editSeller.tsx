...
const EditSellerComponent = ({ id }: { id: string }) => {
  const [form] = Form.useForm();

  const { data: singleStudent, isLoading } = useGetSingleSellerQuery(id, {
    skip: !Boolean(id),
  });

  const [updateSeller, { isLoading: updateLoading }] = useUpdateSellerMutation();
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      const res = await updateSeller({ id, data: values }).unwrap();
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <Typography.Title level={5}>Teacher Information</Typography.Title>
          <Row gutter={16}>
...
export default EditSellerComponent;
