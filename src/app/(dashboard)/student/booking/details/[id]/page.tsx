...
const BookingDetails = ({ params }: any) => {
  ...
  const { data: bookingData = {}, isLoading } = useGetSingleBookingQuery(params?.id, {
    skip: !Boolean(params?.id),
  });

  const ratingFeedBack = bookingData?.ratingFeedback;

  const [updateBooking, { isLoading: bookingLoading }] = useUpdateBookingMutation();
  const [addRating, { isLoading: ratingLoading }] = useAddRatingFeedbackMutation();
          ...
          <Row gutter={16}>
            ...
            >
              <div className="border-2 p-3 flex flex-col justify-center items-center">
                ...
                <Button type="default" danger={defaultValues?.status === 'reject'}>
