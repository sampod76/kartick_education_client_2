...
import { useGetAllBookingQuery, useUpdateBookingMutation } from '@/redux/api/bookingApi';
...
const GeneralUserBooking = () => {
  ...
  const [updateBooking, { isLoading: bookingUpdateLoading }] = useUpdateBookingMutation();
  ...
  const columns = [
    {
      ...
      render: function (data: any) {
        return (
          <>{<Image src={data?.service?.image} width={80} height={50} alt="dd" />}</>
        );
      },
      width: 100,
    },
    ...
    },
    {
      ...
      render: function (data: any) {
        return (
          <>
            {data ? (
              <h1 className="text-base font-normal">Done</h1>
            ) : (
              <h1 className="text-base font-normal">Not payment</h1>
            )}
          </>
        );
      },
